...
class Test_Validation(unittest.TestCase):
    ...
    def test_are_equivalent(self):
        ...
        self.assertTrue(are_equivalent(
            "\\ln (\\exponentialE ^{\\placeholder{}})", "1"))
        self.assertTrue(are_equivalent("\\ln (\\exponentialE^1)", "1"))
        self.assertTrue(are_equivalent(
            "\\ln \\mleft(\\exponentialE \\mright?", "1"))
        self.assertTrue(are_equivalent(
            "\\ln \\mleft(\\exponentialE ^2\\mright?", "2"))
        self.assertTrue(are_equivalent(
            "\\ln \\mleft(\\exponentialE ^{}\\mright?", "1"))
        ...
        self.assertTrue(are_equivalent(
            "\\sin ^{-1}(1)", '\\frac{\\pi }{2}'))  # sin^-1(1), pi/2
        ...
        self.assertTrue(are_equivalent(
            "\log _{10}(100)\cdot \log _{10}(100)", "4"))  # todo
        self.assertTrue(are_equivalent(
            "\\log _3(9)\\cdot \\log _3(3)", "3"))  # todo
        # ambiguous operations due to missing parenthesis around the log's operands
        self.assertFalse(are_equivalent(
            "\log _{10}100\cdot \log _{10}100", "4"))  # todo
        self.assertFalse(are_equivalent(
            "\\log _39\\cdot \\log _33", "3"))  # todo
    ...
    def test_complete_parse(self):
        self.assertEqual(
            str(complete_parse(simplify_latex("\\sin \\mleft(2x\\mright)"))), "sin(2*x)")

    def test_simplify_latex(self):
        self.assertEqual(simplify_latex(
            "\\sin \\mleft(2x\\mright)"), "\sin (2x)")
